...
nn.encode_encrypt([os.path.join(PATH, "model11.json"), os.path.join(PATH, "model11.h5")], he_run_req)

context = nn.get_created_he_context()

with h5py.File(os.path.join(PATH, "x_test5.h5")) as f:
    x_test = np.array(f["x_test5"])
with h5py.File(os.path.join(PATH, "y_test5.h5")) as f:
    y_test = np.array(f["y_test5"])
...
original_datas = np.argmax(plain_predictions, axis=1)

print(labels.shape)
print(original_datas.shape)
utils.assess_results(labels, original_datas)
