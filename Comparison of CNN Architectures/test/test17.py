...
nn.encode_encrypt([os.path.join(PATH, "model17.json"), os.path.join(PATH, "model17.h5")], he_run_req)

context = nn.get_created_he_context()

with h5py.File(os.path.join(PATH, "x_test7.h5")) as f:
    x_test = np.array(f["x_test7"])
with h5py.File(os.path.join(PATH, "y_test7.h5")) as f:
    y_test = np.array(f["y_test7"])
